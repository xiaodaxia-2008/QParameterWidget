...
T GetProperty(const nl::json &schema, const std::string &json_pointer,
  ...
  try {
    ...
    SPDLOG_WARN("failed to get property from {}, exception: {}", json_pointer,
                e.what());
...
QWidget *ParameterItemDelegate::createEditor(QWidget *parent,
  ...
  if (item_type == "integer") {
    ...
    double minimium = GetProperty<double>(m_schema, jp + "/minimium",
                                          std::numeric_limits<double>::min());
    double maximium = GetProperty<double>(m_schema, jp + "/maximium",
